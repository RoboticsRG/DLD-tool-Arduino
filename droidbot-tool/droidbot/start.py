...
def parse_args():
    ...
    parser.add_argument("-policy", action="store", dest="input_policy", default=input_manager.DEFAULT_POLICY,
                        help='Policy to use for test input generation. \n'
                             '\"%s\" -- Explore the app searching for Data Loss problems; \n'
                             '\"%s\" -- Replay the events generated in a previous exploration (A REPLAY_OUTPUT folder is required). \n'
                             %
                             (input_policy.POLICY_DATA_LOSS,
    ...
    parser.add_argument("-policy", action="store", dest="input_policy", default=input_manager.DEFAULT_POLICY,
                        help='Policy to use for test input generation. '
                             ...
                             '  \"%s\" -- Explore UI using a data loss strategy;\n'
                             %
                             (
    ...
    parser.add_argument("-epsilon", action="store", dest="epsilon", type=float,
                        help="The epsilon value to be used during the data loss policiy exploration.\n"
                             "Use values near 0.0 if you want the exploration to be more systematic or near 1.0 if you want it to be more random.")
    ...
    parser.add_argument("-main_activity", action="store", dest="main_activity",
    ...
    if options.epsilon is not None:
        if not 0 <= options.epsilon <= 100:
            print("[WARNING]: value of epsilon must be included in range between 0 and 100.")
            sys.exit(0)
