...
from .input_policy import UtgBasedInputPolicy, UtgNaiveSearchPolicy, UtgGreedySearchPolicy, \
                         ...
                         POLICY_REPLAY, \
                         POLICY_MANUAL, POLICY_MONKEY, POLICY_NONE, POLICY_DATA_LOSS

DEFAULT_POLICY = POLICY_DATA_LOSS
DEFAULT_EVENT_INTERVAL = 2
DEFAULT_EVENT_COUNT = 2250
DEFAULT_TIMEOUT = -1
...
class InputManager(object):
    ...
    def __init__(self, device, app, policy_name, random_input,
        ...
        self.policy = self.get_input_policy(device, app, master, epsilon, scroll_full_down_y)
        self.profiling_method = profiling_method

    def get_input_policy(self, device, app, master, epsilon, scroll_full_down_y):
        if self.policy_name == POLICY_NONE:
            ...
            input_policy = None
        if isinstance(input_policy, UtgBasedInputPolicy) or isinstance(input_policy, DataLossPolicy):
